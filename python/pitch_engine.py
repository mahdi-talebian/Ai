...
# ----------------------------------------------------------------------------
# قفل سراسری رسم نمودار (matplotlib)
# ----------------------------------------------------------------------------
# matplotlib.pyplot یک رابط stateful دارد («figure جاری» یک وضعیت سراسری در
# سطح فرآیند است) و ذاتاً thread-safe نیست. در معماری سرور این پروژه، هر
# job (تحلیل تک‌فایل، مقایسه، صادرات ویدیو) در یک threading.Thread جداگانه
# اجرا می‌شود؛ اگر دو job هم‌زمان وارد کد رسم نمودار (plt.subplots/savefig)
# شوند، ممکن است نمودار یک job در فایل خروجی job دیگر ذخیره شود (باگ واقعی
# مشاهده‌شده: هنگام اجرای هم‌زمان یک تحلیل طولانی و یک مقایسه، نمودار
# مقایسه در فایل PNG تحلیل مقام ذخیره شد). این قفل تضمین می‌کند که در هر
# لحظه فقط یک ترد در حال رسم/ذخیره نمودار با pyplot باشد — بدون این‌که
# منطق تحلیل صوتی (که خودش thread-safe است، چون CPU-bound و بدون state
# سراسری مشترک اجرا می‌شود) کند یا محدود شود.
...
def compute_waveform_and_phrases(
    ...
    if len(envelope) > 0:
        ...
        # --- پیش‌پردازش قبل از آستانه‌گذاری (رفع مشکل «مرزبندی نادقیق فراز
        #     در گفتار واقعی» — تست شد روی نمونهٔ تلاوت واقعی) ---
        #
        # ۱) کلیپ‌کردن نقاط پرت شدید (outlier clipping): چند فریم سکوت
        #    دیجیتالی مطلق (مثلاً ابتدای فایل‌های تولیدشده با TTS، یا سکوت
        #    کامل واقعی) می‌توانند دسی‌بلی به‌مراتب پایین‌تر (نزدیک -inf)
        #    از باقی نواحی «مکث معمولی بین کلمات» داشته باشند. اگر این چند
        #    فریم در خوشه‌بندی K-means شرکت کنند، مرکز خوشهٔ «سکوت» را به‌
        #    شدت به سمت پایین می‌کشند و آستانهٔ نهایی خیلی پایین‌تر از حد
        #    واقعی محاسبه می‌شود — نتیجه: تقریباً کل فایل (از جمله مکث‌های
        #    بین کلمات) به‌اشتباه «صدادار» تشخیص داده می‌شود (دقیقاً همان
        #    باگی که با یک نمونهٔ صوتی TTS واقعی مشاهده و تأیید شد). راه‌حل:
        #    دسی‌بل را در صدک ۱ام (percentile) کف می‌زنیم تا این نقاط پرت
        #    خوشه‌بندی را منحرف نکنند، بدون این‌که مرزهای واقعی فراز تغییر
        #    کنند (چون این کلیپ فقط پایین‌ترین ۱٪ داده را دست می‌زند).
        floor_db = float(np.percentile(db, 1))
        db_clipped = np.maximum(db, floor_db)

        # ۲) هموارسازی میانه با پنجرهٔ کوتاه (~۱۵۰ میلی‌ثانیه): در گفتار/
        #    تلاوت واقعی، بین هجاها/حروف صامت افت‌های خیلی کوتاه و کم‌عمق
        #    دامنه رخ می‌دهد که به‌هیچ‌وجه مکث/نفس واقعی بین فرازها نیستند؛
        #    بدون هموارسازی، این افت‌های ریز می‌توانند نویز باینری ایجاد
        #    کنند که آستانهٔ خوشه‌بندی را نامتعادل می‌کند. میانهٔ متحرک این
        #    افت‌های خیلی کوتاه را حذف می‌کند بدون این‌که مرزهای فراز واقعی
        #    (که معمولاً >0.5 ثانیه سکوت دارند) را محو کند.
        smooth_win = max(1, int(round(0.15 / frame_sec)))
        if smooth_win % 2 == 0:
            smooth_win += 1
        if smooth_win > 1 and len(db_clipped) >= smooth_win:
            from scipy.ndimage import median_filter
            db_smooth = median_filter(db_clipped, size=smooth_win)
        else:
            db_smooth = db_clipped

        # --- ۳) آستانه‌گذاری اصلی: نسبت به اوج انرژی خودِ همین فایل
        #     (peak-relative)، هوشمند و کاملاً خودکار ---
        #
        # ایده: «هر جا بلندی صدا به قلهٔ فایل نزدیک بود = فراز، هر جا موج
        # به‌اندازهٔ کافی افت کرد = مکث». اما محدودهٔ دسی‌بل از فایلی به
        # فایل دیگر خیلی فرق دارد (سطح ضبط، نویز کف، میکروفون، بلندگو) —
        # پس یک عدد ثابت (مثلاً «۲۰ دسی‌بل زیر قله») برای همهٔ فایل‌ها کار
        # نمی‌کند. به‌جایش:
        #   peak_ref  = صدک ۹۵ام دسی‌بل هموارشده — نمایندهٔ «قلهٔ واقعی
        #               صدا»، اما مقاوم به یک ترقهٔ لحظه‌ای/کلیک غیرواقعی
        #               (برخلاف np.max خام که می‌تواند با یک فریم پرت خراب شود).
        #   floor_ref = صدک ۱۰ام — نمایندهٔ «کف نویز/سکوت واقعی» همان فایل.
        #   dynamic_range = peak_ref - floor_ref، یعنی کنتراست واقعی صدا در
        #               برابر سکوت، مخصوص همین فایل.
        #   margin_db = فاصلهٔ آستانه از قله؛ به‌نسبت dynamic_range خودِ
        #               فایل تنظیم می‌شود (نه عدد ثابت): فایل‌های باکنتراست
        #               بالا (صدای واضح روی سکوت عمیق) آستانه را دورتر از
        #               قله می‌گیرند (فراز پهن‌تر/طبیعی‌تر شامل افت‌وخیزهای
        #               طبیعی صدا)؛ فایل‌های کم‌کنتراست (نویز محیط بالا)
        #               آستانه را نزدیک‌تر به قله نگه می‌دارند تا نویز
        #               به‌اشتباه «فراز» حساب نشود. بازهٔ مجاز margin
        #               (۶ تا ۱۸ دسی‌بل) از آزمایش روی نمونه‌های واقعی/TTS
        #               به‌دست آمده.
        peak_ref = float(np.percentile(db_smooth, 95))
        floor_ref = float(np.percentile(db_smooth, 10))
        dynamic_range = max(1e-6, peak_ref - floor_ref)
        margin_db = float(np.clip(0.35 * dynamic_range, 6.0, 18.0))
        threshold_db = peak_ref - margin_db
        is_loud = db_smooth > threshold_db

        # --- اعتبارسنجی ایمنی: اگر آستانهٔ نسبت‌به‌قله به نتیجهٔ کاملاً
        #     بی‌فایده رسید (مثلاً کل فایل صدادار یا کل فایل ساکت تشخیص
        #     داده شد — که می‌تواند در فایل‌های خیلی کوتاه/تک‌نت یا خیلی
        #     نویزی رخ دهد)، به روش قدیمی‌تر و پایدارتر K-means/Otsu
        #     دوقطبی برمی‌گردیم؛ این فقط fallback است، نه الگوریتم اصلی. ---
        frac_loud = float(np.mean(is_loud))
        if frac_loud <= 0.02 or frac_loud >= 0.98:
            threshold_db = _otsu_threshold(db_smooth)
            # حاشیهٔ کوچک امن پایین‌تر از آستانه — تا خودِ لبهٔ محو شدن صدا
            # (fade-out طبیعی نت) به‌اشتباه به‌عنوان مکث حساب نشود.
            is_loud = db_smooth > (threshold_db - 2.0)
...
def dtw_align(seq_a: np.ndarray, seq_b: np.ndarray, band_radius: int = None):
    ...
    # اگر band_radius داده شده باشد ولی کوچک‌تر از تفاوت طول دو دنباله
    # باشد، از نظر ریاضی هیچ مسیر معتبری به گوشهٔ پایانی (n, m) وجود
    # نخواهد داشت (چون برای رسیدن از (0,0) به (n,m) باید حداقل |n-m| قدم
    # در یک جهت برداشته شود) — نتیجه: D[n,m]=inf و شباهت محاسبه‌شده نامعتبر
    # (inf/NaN) می‌شود. این باگ واقعی با یک مقایسهٔ واقعی بین دو فراز با
    # تعداد نت بسیار متفاوت (مثلاً ۵۰ در برابر ۵ نت) بازتولید و تأیید شد.
    # راه‌حل: کف امن برای band_radius، مستقل از مقدار ورودی.
    if band_radius is not None:
        band_radius = max(band_radius, abs(n - m))
